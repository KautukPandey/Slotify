        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-6 lg:col-span-2 space-y-5">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-xl bg-brand-600 flex items-center justify-center text-white font-bold text-xl shadow-sm">
                {getInitials(user?.name)}
              </div>
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 capitalize">{user?.name}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">Customer account</p>
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 text-sm">
                ...
                <span className="sm:col-span-2 text-slate-800 dark:text-slate-200">{user?.email}</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 text-sm">
                <span className="text-slate-400 dark:text-slate-500 font-medium">Location</span>
                <span className="sm:col-span-2 text-slate-800 dark:text-slate-200 capitalize">{user?.city || "Not set"}</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 text-sm">
