...
const Login = () => {
  ...
  // Handle registration redirect state
  useEffect(() => {
    if (location.state) {
      ...
      // Clear location state to prevent showing the alert again on refresh
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 dark:bg-zinc-950 p-4 transition-colors duration-300">
      ...
      <div className="w-full max-w-[420px] bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 rounded-2xl shadow-xl shadow-slate-100/50 dark:shadow-none p-8 relative overflow-hidden transition-all duration-300 z-10">
        ...
        {successMessage && (
          <div className="mb-5 p-3.5 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 text-emerald-700 dark:text-emerald-400 text-sm rounded-xl flex items-center gap-2.5 animate-fadeIn">
        ...
        {errors.apiError && (
          <div className="mb-5 p-3.5 bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50 text-red-700 dark:text-red-400 text-sm rounded-xl flex items-center gap-2.5 animate-fadeIn">
            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center">
            <input
              id="remember-me"
              type="checkbox"
              className="h-4 w-4 rounded border-slate-300 dark:border-zinc-700 text-violet-600 focus:ring-violet-500/20 focus:ring-offset-0 dark:bg-zinc-800 bg-transparent transition-all duration-150 cursor-pointer"
            />
            <label 
              htmlFor="remember-me" 
              className="ml-2 text-xs text-slate-500 dark:text-zinc-400 select-none cursor-pointer"
            >
              Keep me signed in
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className={`w-full py-2.5 px-4 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition-all duration-200 focus:outline-none focus:ring-2
              ${isLoading 
        ...
        <div className="relative my-6 flex items-center justify-center">
          ...
          <span className="relative px-3 bg-white dark:bg-zinc-900 text-[11px] font-medium text-slate-400 dark:text-zinc-500 uppercase tracking-wider">
            Or continue with
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3.5">
          <button
            type="button"
            className="flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl border border-slate-200 dark:border-zinc-800 bg-transparent hover:bg-slate-50 dark:hover:bg-zinc-800/50 text-slate-700 dark:text-zinc-300 font-medium text-xs transition-all duration-200 active:scale-[0.98]"
